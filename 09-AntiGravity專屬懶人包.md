...
## 四、Obsidian 筆記工作流（不使用 MCPVault）

本版移除 Obsidian MCPVault 安裝與 MCP 註冊。Obsidian 仍可作為專案駕駛艙與踩坑筆記的位置，但由使用者或 AI 透過一般檔案操作、手動整理或其他已授權方式維護，不把 MCPVault 當成 Anti-Gravity 必裝工具。
...
- 不要求安裝 `@bitbonsai/mcpvault`。
- 不要求在 Anti-Gravity 註冊 Obsidian MCP。
- 不把 vault 路徑、私人筆記清單或敏感內容寫進公開 repo。
- 若專案需要同步 Obsidian，先確認實際授權與檔案位置，再只更新和本次任務相關的筆記。
...
- 專案要引用的圖片請放在專案 `assets/` 或經確認的素材資料夾。
...
Anti-Gravity 可使用專案根目錄的 `ANTIGRAVITY.md` 作為 AI 工作規則入口。它應記錄固定規則、路徑、專案邊界與 Do / Don't；進度、踩坑、每日紀錄應放在專案筆記或 Obsidian 專案駕駛艙。本版不要求透過 MCPVault 自動讀寫。
...
2. 讀取或請使用者提供專案筆記 / Obsidian 專案駕駛艙重點。
...
2. 更新專案筆記或 Obsidian 專案駕駛艙：完成事項、下一步、踩坑。
...
- 涉及檔案操作時回報完整產出位置。
- 使用 PowerShell 語法。
- 開工時讀本檔、讀專案筆記或 Obsidian 駕駛艙重點、檢查 Git 狀態。
- 收工時更新專案筆記，必要時更新本檔，檢查 diff 後只提交相關檔案。
- 不把每日流水帳寫進本檔。
...
| 需要 Obsidian 紀錄但未設定 MCPVault | 本版不使用 MCPVault；請改用一般專案筆記、手動整理，或在明確授權後處理指定檔案 |
